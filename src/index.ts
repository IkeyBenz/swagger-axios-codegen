...
export async function codegen(params: ISwaggerOptions) {
  ...
  const apiSource = options.useCustomerRequestInstance ? customerServiceHeader(options, swaggerSource.basePath) : serviceHeader(options, swaggerSource.basePath);
  ...
  if (options.multipleFileMode) {
    ...
    Object.entries(requestCodegen(swaggerSource.paths, isV3, options)).forEach(([className, requests]) => {
      ...
      allImport.push('getConfigs', 'axios');
